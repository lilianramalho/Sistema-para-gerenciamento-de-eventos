...
if(!empty($filtroSelecionado) && $filtroSelecionado == "Alfabética"){
	...
	$sql = ("SELECT * FROM tbevento WHERE eventoexiste = '1' ORDER BY nomeevento ASC");

}else if(!empty($filtroSelecionado) && $filtroSelecionado == "Inversa alfabética"){

	$sql = ("SELECT * FROM tbevento WHERE eventoexiste = '1' ORDER BY nomeevento DESC");

}else if(!empty($filtroSelecionado) && $filtroSelecionado == "Primeiros cadastrados"){

	$sql = ("SELECT * FROM tbevento WHERE eventoexiste = '1' ORDER BY codevento ASC");
...
if ($data = mysqli_query($conn, $sql)){
	while ($row = mysqli_fetch_assoc($data)){
		...
		$info = array($row['codevento'],$row['nomeevento'],$row['descricaoevento'],$dataEvento2,$dataEvento4,$row['horainicioevento'],$row['horafimevento'],$imagem,$cor,$row['codsala']);
		$str = implode('~', $info);
