...
<div id="modalBody8">
    <div id="modalContent8">
        ...
        <form action="inserirCadastroAgenda.php" class="hora" id="hora" method="post">
            ...
            <input type="text" id="solicitante" class="input-form" name="solicitante" placeholder="Solicitante">

            <input type="text" id="instituicao" class="input-form" name="instituicao" placeholder= "Instituição" style="display: none;">
            <input type="file" id="instituicaoArquivo" class="input-form" name="instituicaoArquivo" style="display: none;">
            ...
            <div id="group" style="display: flex;flex-direction: column;align-items: flex-start;width: 100%;">
                <label style="font-size: 13px;color: #777;margin-bottom: 5px;margin-left: 10px;">É uma instituição?</label>
                <div id="group" style="width: auto;padding-left: 10px;">

                    <div id="group" style="width: auto;margin-right: 20px;font-size: 13px;">
                        <input type="radio" name="instituicao" value="Sim" id="radioInstituicao1" onclick="verRadio(1);">
                        <label for="instituicao-cadastro" id="labelSim">Sim</label>
                    </div>
                    

                    <div id="group" style="width: auto;font-size: 13px;">
                        <input type="radio" name="instituicao" value="Não" id="radioInstituicao2" onclick="verRadio(2);">
                        <label for="instituicao-cadastro" id="labelNao">Não</label>
                    </div>
                </div>
            </div>

            <script type="text/javascript">
                function verRadio(valor){
                    if(valor == 1){
                        $("#instituicao").css({display:"block"});
                        $("#instituicaoArquivo").css({display:"block"});
                    }else{
                        $("#instituicao").css({display:"none"});                        
                        $("#instituicaoArquivo").css({display:"none"});                        
                    }
                }
            </script>
...
<!--             <div id="containerFormAgenda2" style="display: none;">
 -->            



           <input type='text' class='buscaAgenda' placeholder='Buscar' style="display: none;width:200px;height:20px;">

                <div class="listagemSalasAgenda" style="display: none;"></div>

                 <script type="text/javascript">

                        $(".buscaAgenda").keyup(function(){     
                            buscar($('.buscaAgenda').val());
                        }); 
 ...
 -->
            ...
            <div class="group2" style="padding: 15px 30px 15px 15px;">
               <!--  <input type="button" class="butn pop_up_fechar" name="fechar" value="Cancelar" onclick="fecharModal8()" style="background-color: transparent;color: #999;">  -->               
                <input type="button" id="btn-Agendar" class="butn" name="enviar" value="Próximo" onclick="proxModalAgenda(1)">
            </div>
           ...
           <script>
             function proxModalAgenda(valor){
                if(valor == 1){  
                    ...
                    if($('#atividade').val().length > 0 && $('#solicitante').val().length > 0 && $("#radioInstituicao1").is(':checked')
                        || $("#radioInstituicao2").is(':checked')){

                    $('#secaoPopUpAgenda1').css({display:"none"});
                    $('.buscaAgenda').css({display:"flex"});
                    // $('.hora').css({display:"none"});
                    $('#group2').css({display:"none"});
                    $('.listagemSalasAgenda').css({display:"flex"});
                    // $('#containerFormAgenda').css({display:"none"});
                    // $('#containerFormAgenda2').css({display:"flex"});
                    $('#group3').css({display:"flex"});
                    document.getElementById("tituloHeaderPopUp").innerHTML = "Escolha um espaço";
                    listagemSalas();
                    ...
                    $('#secaoPopUpAgenda1').css({display:"block"});
                    $('.buscaAgenda').css({display:"none"});
                    $('.hora').css({display:"flex"});                    
                    $('#group2').css({display:"flex"});
                    $('.listagemSalasAgenda').css({display:"none"});
                    $('#containerFormAgenda').css({display:"flex"});
                    $('#containerFormAgenda2').css({display:"none"});                    
                    $('#group3').css({display:"none"});            
                    document.getElementById("tituloHeaderPopUp").innerHTML = "Solicitação agendamento";
                    ...
                    $.post($("#hora").attr("action"), $("#hora :input").serializeArray(), function(info){
                        ...
                        if(info == " agendado "){
                            fecharModal8();
                              ...
                              'A solicitação de agendamento foi concluída com sucesso.',
                              'success'
                              )   
                        }else{
                            swal(
            ...
            <div id="group3" style="width: 100%;display: none;padding: 15px 30px 15px 15px; justify-content: flex-end;align-items: flex-end;">
                ...
                <input type="button" id="btn-Agendar" class="butn" name="enviar" value="Próximo" onclick="proxModalAgenda(3)">
            </div>
